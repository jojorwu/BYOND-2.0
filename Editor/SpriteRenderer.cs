...
namespace Editor
{
    public class SpriteRenderer : IDisposable
    {
        ...
        public SpriteRenderer(GL gl)
        {
            ...
            if (success == 0)
            {
                throw new Exception($"Error linking shader program: {_gl.GetProgramInfoLog(_shaderProgram)}");
            }
        ...
        public unsafe void Draw(uint textureId, Vector2 position, Vector2 size, float rotate, Matrix4x4 projection)
        {
            _gl.UseProgram(_shaderProgram);

            Matrix4x4 model = Matrix4x4.CreateScale(new Vector3(size.X, size.Y, 1.0f));
            model = Matrix4x4.CreateTranslation(new Vector3(-size.X * 0.5f, -size.Y * 0.5f, 0.0f)) * model;
            model = Matrix4x4.CreateRotationZ(rotate) * model;
            model = Matrix4x4.CreateTranslation(new Vector3(size.X * 0.5f, size.Y * 0.5f, 0.0f)) * model;
            model = Matrix4x4.CreateTranslation(new Vector3(position.X, position.Y, 0.0f)) * model;
