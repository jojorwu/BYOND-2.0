...
internal partial class DMCodeTree {
    public abstract class VarNode : INode {
        ...
        public abstract bool TryDefineVar(DMCompiler compiler, int pass, ScopeMode scopeMode);
    ...
    private class ObjectVarNode(DreamPath owner, DMASTObjectVarDefinition varDef) : VarNode {
        ...
        public override bool TryDefineVar(DMCompiler compiler, int pass, ScopeMode scopeMode) {
            if (_defined)
                return true;
            ...
            if (IsStatic) {
                return HandleGlobalVar(compiler, dmObject, pass, scopeMode);
            } else {
        ...
        private bool HandleGlobalVar(DMCompiler compiler, DMObject dmObject, int pass, ScopeMode scopeMode) {
            var expression = varDef.Value ?? new DMASTConstantNull(varDef.Location);
            if (!TryBuildValue(new(compiler, dmObject, compiler.GlobalInitProc), expression, varDef.Type, scopeMode, out var value))
                return false;
    ...
    private class ObjectVarOverrideNode(DreamPath owner, DMASTObjectVarOverride varOverride) : VarNode {
        ...
        public override bool TryDefineVar(DMCompiler compiler, int pass, ScopeMode scopeMode) {
            ...
            if (!TryBuildValue(new(compiler, dmObject, compiler.GlobalInitProc), expression, variable.Type, scopeMode, out var value))
                return false;
    ...
    private class ProcGlobalVarNode(DreamPath owner, DMProc proc, DMASTProcStatementVarDeclaration varDecl) : VarNode {
        ...
        public override bool TryDefineVar(DMCompiler compiler, int pass, ScopeMode scopeMode) {
            ...
            if (expression != null) {
                ...
                if (!TryBuildValue(exprContext, expression, varDecl.Type, scopeMode, out value))
                    return false;
            }
    ...
    public void AddObjectVarOverride(DreamPath owner, DMASTObjectVarOverride varOverride) {
        ...
        if (varOverride.VarName == "parent_type") {
            ...
            node.ParentTypeLocation = varOverride.Location;
