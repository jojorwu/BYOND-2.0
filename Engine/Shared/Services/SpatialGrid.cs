    ...
    public class SpatialGrid : IDisposable, IShrinkable
    {
        ...
        public void Add(IGameObject obj)
        {
            while (true)
            {
                ...
                var oldKeyNullable = obj.CurrentGridCellKey;

                if (oldKeyNullable != null)
                {
                    var oldKey = oldKeyNullable.Value;
                    ...
                    if (CompareKeys(oldKey, key) < 0)
                    {
                        ...
                        try {
                            ...
                            try {
                                ...
                                if (obj.CurrentGridCellKey != oldKey) continue;
                                RemoveInternal(obj, oldCell);
                                AddInternal(obj, cell, key);
                                return;
                            } finally { cell.Lock.ExitWriteLock(); }
                        } finally { oldCell.Lock.ExitWriteLock(); }
                    }
                    else
                    {
                        cell.Lock.EnterWriteLock();
                ...
                else
                {
                    ...
                    try
                    {
                        if (obj.CurrentGridCellKey != null) continue;
                        AddInternal(obj, cell, key);
                        return;
                    }
                    finally
        ...
        {
            // Verify that the object actually belongs to this cell and is not already removed
            // Check if it's actually in this linked list by checking Head or Prev/Next
            if (cell.Head != obj && obj.PrevInGridCell == null && obj.NextInGridCell == null && obj.CurrentGridCellKey == null)
            {
        ...
        public void QueryBox(Box2l box, Action<IGameObject> callback)
        {
            ...
            if ((double)(endGX - startGX + 1) * (endGY - startGY + 1) > 1000000) return;
