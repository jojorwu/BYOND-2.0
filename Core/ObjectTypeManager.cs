...
namespace Core
{
    ...
    {
        public ConcurrentDictionary<string, ObjectType> ObjectTypes { get; } = new();

        public void RegisterObjectType(ObjectType objectType)
        {
            if (ObjectTypes.ContainsKey(objectType.Name))
            {
            ...
            {
                if (ObjectTypes.TryGetValue(objectType.ParentName, out var parentType))
                {
            ...
            foreach (var childType in ObjectTypes.Values)
            {
        ...
        {
            ObjectTypes.TryGetValue(name, out var objectType);
            return objectType;
        }
