                    ...
                    while (thread.State == DreamThreadState.Running)
                    {
                        ...
                        for (int i = 0; i < 8; i++)
                        {
                            if (thread.State != DreamThreadState.Running) break;
                                if (state.PC >= state.BytecodeArray.Length - 8)
                            {
                            ...
                            switch (opcode)
                            {
                                ...
                                case Opcode.PopReference:
                                    {
                                        var refType = (DMReference.Type)state.BytecodePtr[state.PC++];
                                        if (refType == DMReference.Type.Field || refType == DMReference.Type.SrcField || refType == DMReference.Type.SrcProc) state.PC += 4;
