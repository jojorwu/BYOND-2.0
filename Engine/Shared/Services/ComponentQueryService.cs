...
[EngineService(typeof(IComponentQueryService))]
public class ComponentQueryService : EngineService, IComponentQueryService, IDisposable, IShrinkable
{
    ...
    private class QueryResult : IEntityQuery, IDisposable
    {
        ...
        public QueryEnumerator GetEnumerator() => new QueryEnumerator(_archetypes);
    ...
    public IEnumerable<IGameObject> Query<T>() where T : class, IComponent => Query(typeof(T));

    public IEnumerable<IGameObject> Query(params ReadOnlySpan<Type> componentTypes) => GetQuery(componentTypes);
