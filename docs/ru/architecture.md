...
### Конфигурация
Настройки сервера управляются через файл `server_config.json`. Это позволяет операторам легко настраивать такие параметры, как порт сервера, настройки региональной обработки и другие опции времени выполнения, без перекомпиляции кода.
